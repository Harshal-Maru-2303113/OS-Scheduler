...
class EventHandlers:
    ...
    def run_and_plot(self):
        ...
        if algo not in ("HPF","FCFS","RR","SRTN","MLFQ","SJF", "Lottery"):
        ...
        try:
            ...
            quantum = 1.0
        try:
            aging_interval = float(self.app.aging_interval_var.get())
        except Exception as e:
            aging_interval = 3
            print("Aging Interval Error:", e)
        try:
            aging_increment = int(self.app.aging_increment_var.get())
        except Exception as e:
            aging_increment = 0
            print("Aging Increment Error:", e)
        ...
        try:
            if algo == "FCFS":
                ...
                timeline, stats = sched.hpf_with_aging(context_switch=context, aging_interval=aging_interval, aging_increment=aging_increment)
            elif algo == "Lottery":
                timeline, stats = sched.lottery(quantum=quantum,context_switch=context)
            elif algo == "RR":
