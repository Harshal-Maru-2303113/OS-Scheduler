...
class ControlsFrame:
    def __init__(self, parent, app):
        ...
        ttk.OptionMenu(sched_frame, self.algorithm_var, "SELECT", "SJF", "HPF", "FCFS", "RR", "SRTN", "MLFQ", "Lottery").grid(row=0, column=1, sticky="w", padx=(6,12))
        ...
        #---------Aging Settings--------
        aging_frame = ttk.LabelFrame(parent, text="Aging Settings(For HPF)", padding=10)
        aging_frame.pack(fill=X, pady=(0,10))

        ttk.Label(aging_frame, text="Aging Interval:").grid(row=0, column=0, sticky="w")
        self.aging_interval_var = StringVar(value="5")
        ttk.Entry(aging_frame, textvariable=self.aging_interval_var, width=8).grid(row=0, column=1, sticky="w", padx=(6,12))

        ttk.Label(aging_frame, text="Aging Increment:").grid(row=1, column=0, sticky="w")
        self.aging_increment_var = StringVar(value="0")
        ttk.Entry(aging_frame, textvariable=self.aging_increment_var, width=8).grid(row=1, column=1, sticky="w", padx=(6,12))
